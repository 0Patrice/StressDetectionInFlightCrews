...
if __name__ == '__main__':
    ...
    data_folder = "/home/lori_pa/ml/MRFmV"
    learned_folder = "/home/lori_pa/ml/0_learned"

    # ECG Signal in mV -> 1mV = 75 Steps with a range (0, 246)
    ecg_norm = 75/246
    ...
    def closure():
        ...
        with open(rf'lossNEO_i{i}.csv', 'a') as f:
            writer = csv.writer(f)
    ...
    for i in range(opt.steps):
        ...
        print(f'{bcolors.OKBLUE}STEP: {i}{bcolors.ENDC}\n')
        ...
        for root, subdirectories, files in os.walk(data_folder, topdown=True):
            for file in files:
                ...
                train_input = torch.tensor([data[:, 1]*ecg_norm]).to(device)
                train_target = torch.tensor([data[:, 2]*slt_norm]).to(device)
                ...
                newDir = cdDir[0] + "/0_learned/" + file
                shutil.move(file_path, newDir)
                logging.info(f'Moved {file} to {newDir}')

                logging.info(f'<< End')

        logging.info(f'Compleated all files for Step {i}')
        ...
        for root, subdirectories, files in os.walk(valDataFolder, topdown=True):
            for file in files:
                ...
                test_input = torch.tensor([valdata[:, 1]*ecg_norm]).to(device)
                test_target = torch.tensor([valdata[:, 2]*slt_norm]).to(device)
                ...
                with torch.no_grad():
                    ...
                    with open(rf'lossValNEO_i{i}.csv', 'a') as f:
                        writer = csv.writer(f)
                        writer.writerow(fields)
